...
class TemperatureMachine(StateMachine):
    ...
    def updateLights(self):
        ...
        if(self.current_state.id == 'heat'):
            # Condition for Thermostat set to heath
            if(temp >= self.setPoint):
                # Turn on red light solid because temperature already above set point
                redLight.on()
            else:
                # Pulse red light because temperature is below set point and heat on
                redLight.pulse()
        elif(self.current_state.id == 'cool'):
            # Thermostat is set to cool
            if(temp <= self.setPoint):
                # Blud light solid because temperature already below set point
                blueLight.on()
            else:
                # Blue light pulse because temperature above set point and cool on
                blueLight.pulse()
        else:
            # No light activity because state machine is off
            pass
    ##
